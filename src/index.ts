...
import { loadCalendars, hasCalendars, CalendarEvent } from "./calendar.js";
...
if (!VIKUNJA_URL || !VIKUNJA_TOKEN) {
  console.error(
    "Error: VIKUNJA_URL and VIKUNJA_TOKEN environment variables are required."
  );
  console.error("  VIKUNJA_URL   = https://vikunja.lindenlion.net");
  console.error(
    "  VIKUNJA_TOKEN = (create one under Settings > API Tokens in Vikunja)"
  );
  process.exit(1);
}
...
function formatProject(p: VikunjaProject): string {
  const parts = [
    `#${p.id} ${p.title}`,
...
function formatCalendarAgenda(sources: Awaited<ReturnType<typeof loadCalendars>>): string {
  const byDay = new Map<string, { label: string; lines: string[] }>();

  for (const source of sources) {
    if (source.error) {
      const key = "error";
      if (!byDay.has(key)) byDay.set(key, { label: "⚠️ Errors", lines: [] });
    ...
    for (const event of source.events) {
      ...
      byDay.get(dateKey)!.lines.push(
        `  [${source.calendarName}] ${formatEventTime(event)}  ${event.summary}` +
        (event.location ? `  📍 ${event.location}` : "")
      );
    }
  }

  if (byDay.size === 0) return "  No events in this range.";
...
function createServer(): McpServer {
  ...
  server.tool(
    "list_projects",
    "List all projects (lists) in Vikunja. Returns project IDs, titles, and metadata.",
    {},
    async () => {
  ...
  server.tool(
    "update_project",
    "Update an existing project's title, description, archive status, or color.",
    {
  ...
  server.tool(
    "list_tasks",
    `List tasks across all projects, with optional filtering and search.
...
Sort options: id, title, done, done_at, due_date, created, updated, priority, position`,
  ...
  server.tool(
    "get_task",
    "Get full details of a specific task by ID, including description and comments.",
    ...
    async ({ task_id }) => {
      ...
      if (comments.length) {
        ...
        text += comments
          .map(
            (c) =>
              `    ${c.author?.name || c.author?.username || "?"} (${c.created}): ${c.comment}`
          )
          .join("\n");
      }
      return { content: [{ type: "text", text }] };
  ...
  server.tool(
    "create_task",
    "Create a new task in a project. Priority: 0=unset, 1=low, 2=medium, 3=high, 4=urgent, 5=DO NOW. Dates must be ISO 8601 (e.g. '2026-04-10T09:00:00Z').",
    {
  ...
  server.tool(
    "update_task",
    "Update a task's title, description, status, priority, dates, or progress.",
    {
  ...
  server.tool(
    "complete_task",
    "Mark a task as done.",
    { task_id: z.number().describe("The task ID to complete") },
    async ({ task_id }) => {
  ...
  server.tool(
    ...
    { task_id: z.number().describe("The task ID to reopen") },
    async ({ task_id }) => {
  ...
  server.tool(
    "get_calendar_events",
    `Get events from all configured calendars (local .ics files and webcal subscriptions) within a date window.
...
The server also exposes a machine-readable iCal feed at GET /calendar.ics that can be subscribed to by calendar apps.`,
  ...
  server.tool(
    "create_relation",
    `Create a relation between two tasks. This is how you create subtasks, blocking dependencies, etc.
